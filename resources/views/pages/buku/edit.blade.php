            ...
            <div class="col-12">
                <div class="card mt-2 mt-md-3 shadow-sm border-radius-lg">
                    <div class="card-header pb-0 p-3 bg-transparent border-bottom">
                        <h6 class="mb-0 font-weight-bolder text-dark">Form Edit Data Buku</h6>
                    </div>
                        ...
                        <form action="{{ route('buku.update', $buku->id) }}" method="POST">
                            ...
                            <div class="row">
                                <div class="col-md-4 mb-3">
                                    ...
                                    <input type="number" name="tahun_terbit" id="tahun_terbit" value="{{ old('tahun_terbit', $buku->tahun_terbit) }}" class="form-control" placeholder="Contoh: 2024" required>
                                    @error('tahun_terbit')
                                ...
                                <div class="col-md-8 mb-3">
                                    ...
                                    <select name="user_id" id="user_id" class="form-control select2" data-placeholder="-- Pilih Dosen --" required>
                                        ...
                                        @foreach ($users as $user)
                                            <option value="{{ $user->id }}" {{ old('user_id', $buku->user_id) == $user->id ? 'selected' : '' }}>
                            ...
                            <div class="mb-3">
                                ...
                                <textarea name="judul_buku" id="judul_buku" rows="2" class="form-control" placeholder="Masukkan judul lengkap buku" required>{{ old('judul_buku', $buku->judul_buku) }}</textarea>
                                @error('judul_buku')
                                    <div class="text-danger text-xs mt-1">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="row">
                                <div class="col-md-6 mb-3">
                                    ...
                                    <input type="text" name="isbn" id="isbn" value="{{ old('isbn', $buku->isbn) }}" class="form-control" placeholder="Contoh: 978-623-123-456-7" required>
                                    @error('isbn')
                                        <div class="text-danger text-xs mt-1">{{ $message }}</div>
                                    @enderror
                                </div>
                                <div class="col-md-6 mb-3">
                                    ...
                                    <input type="text" name="penerbit" id="penerbit" value="{{ old('penerbit', $buku->penerbit) }}" class="form-control" placeholder="Masukkan nama penerbit" required>
                                    @error('penerbit')
                                        <div class="text-danger text-xs mt-1">{{ $message }}</div>
                                    @enderror
                                </div>
                            ...
                            <div class="mb-3">
                                ...
                                <textarea name="dokumen" id="dokumen" rows="3" class="form-control" placeholder="Masukkan link Google Drive dokumen buku / cover / bukti terbit (contoh: https://drive.google.com/file/d/...)" required>{{ old('dokumen', $buku->dokumen) }}</textarea>
                                @error('dokumen')
                                    <div class="text-danger text-xs mt-1">{{ $message }}</div>
                                @enderror
                            </div>
