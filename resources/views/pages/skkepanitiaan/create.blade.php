...
@section('title', 'Tambah SK Kepanitiaan')
            ...
            <div class="col-12">
                <div class="card mt-2 mt-md-3 shadow-sm border-radius-lg">
                    <div class="card-header pb-0 p-3 bg-transparent border-bottom">
                        <h6 class="mb-0 font-weight-bolder text-dark">Form Tambah SK Kepanitiaan</h6>
                    </div>
                    <div class="card-body p-3 p-md-4">
                        <form action="{{ route('skkepanitiaan.store') }}" method="POST">
                            @csrf
                            ...
                            <div class="mb-3">
                                ...
                                <select name="kategorysk_id" id="kategorysk_id" class="form-control select2" data-placeholder="-- Pilih Kategori SK --">
                                    ...
                                    @foreach ($kategorysk as $item)
                                        <option value="{{ $item->id }}" {{ old('kategorysk_id') == $item->id ? 'selected' : '' }}>{{ $item->kategory_sk }}</option>
                                    @endforeach
                                </select>
                                @error('kategorysk_id')
                                    <div class="text-danger text-xs mt-1">{{ $message }}</div>
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" name="nomor_sk" id="nomor_sk" value="{{ old('nomor_sk') }}" class="form-control" placeholder="Masukkan nomor SK">
                                @error('nomor_sk')
                                    <div class="text-danger text-xs mt-1">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <textarea name="dokumen" id="dokumen" rows="3" class="form-control" placeholder="Masukkan link Google Drive dokumen SK (contoh: https://drive.google.com/file/d/...)">{{ old('dokumen') }}</textarea>
                                @error('dokumen')
                                    <div class="text-danger text-xs mt-1">{{ $message }}</div>
                                @enderror
                            </div>
