...
@section('title', 'Edit SK Penguji Tugas Akhir')
            ...
            <div class="col-12">
                <div class="card mt-2 mt-md-3 shadow-sm border-radius-lg">
                    <div class="card-header pb-0 p-3 bg-transparent border-bottom">
                        <h6 class="mb-0 font-weight-bolder text-dark">Form Edit SK Penguji Tugas Akhir</h6>
                    </div>
                    <div class="card-body p-3 p-md-4">
                        <form action="{{ route('skpengujitugasakhir.update', $skpengujitugasakhir) }}" method="POST">
                            @csrf
                            @method('PUT')
                            ...
                            <div class="mb-3">
                                ...
                                <select name="tahunakademik_id" id="tahunakademik_id" class="form-control select2" data-placeholder="-- Pilih Tahun Akademik --" required>
                                    ...
                                    @foreach ($tahunakademik as $item)
                                        <option value="{{ $item->id }}" {{ old('tahunakademik_id', $skpengujitugasakhir->tahunakademik_id) == $item->id ? 'selected' : '' }}>{{ $item->tahun_akademik }}</option>
                                    @endforeach
                                </select>
                            ...
                            @php
                                $selectedUserIds = old('user_ids', $skpengujitugasakhir->users->pluck('id')->toArray());
                            @endphp
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" name="nomor_sk" id="nomor_sk" value="{{ old('nomor_sk', $skpengujitugasakhir->nomor_sk) }}" class="form-control" placeholder="Masukkan nomor SK" required>
                                @error('nomor_sk')
                                    <div class="text-danger text-xs mt-1">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="row">
                                <div class="col-md-6 mb-3">
                                    ...
                                    <input type="text" name="nama_mahasiswa" id="nama_mahasiswa" value="{{ old('nama_mahasiswa', $skpengujitugasakhir->nama_mahasiswa) }}" class="form-control" placeholder="Masukkan nama mahasiswa" required>
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    <input type="text" name="npm" id="npm" value="{{ old('npm', $skpengujitugasakhir->npm) }}" class="form-control" placeholder="Masukkan NPM mahasiswa" required>
                                    @error('npm')
                                        <div class="text-danger text-xs mt-1">{{ $message }}</div>
                                    @enderror
                                </div>
                            ...
                            <div class="mb-3">
                                ...
                                <input type="date" name="tanggal_sk" id="tanggal_sk" value="{{ old('tanggal_sk', $skpengujitugasakhir->tanggal_sk ? \Carbon\Carbon::parse($skpengujitugasakhir->tanggal_sk)->format('Y-m-d') : '') }}" class="form-control" required>
                                @error('tanggal_sk')
                                    <div class="text-danger text-xs mt-1">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <textarea name="dokumen" id="dokumen" rows="3" class="form-control" placeholder="Masukkan link Google Drive dokumen SK (contoh: https://drive.google.com/file/d/...)" required>{{ old('dokumen', $skpengujitugasakhir->dokumen) }}</textarea>
                                @error('dokumen')
                                    <div class="text-danger text-xs mt-1">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="d-flex align-items-center gap-2 pt-2">
                                ...
                                <a href="{{ route('skpengujitugasakhir.index') }}" class="btn btn-danger text-white text-uppercase px-4 mb-0">
