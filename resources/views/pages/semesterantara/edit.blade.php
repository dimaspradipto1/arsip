...
@section('title', 'Edit LPJ Semester Antara')
            ...
            <div class="col-12">
                <div class="card mt-2 mt-md-3 shadow-sm border-radius-lg">
                    <div class="card-header pb-0 p-3 bg-transparent border-bottom">
                        <h6 class="mb-0 font-weight-bolder text-dark">Form Edit LPJ Semester Antara</h6>
                    </div>
                    <div class="card-body p-3 p-md-4">
                        <form action="{{ route('semesterantara.update', $semesterantara->id) }}" method="POST">
                            @csrf
                            @method('PUT')
                            ...
                            <div class="mb-3">
                                ...
                                <select name="tahunakademik_id" id="tahunakademik_id" class="form-control select2" data-placeholder="-- Pilih Tahun Akademik --" required>
                                    ...
                                    @foreach ($tahunakademik as $item)
                                        <option value="{{ $item->id }}" {{ old('tahunakademik_id', $semesterantara->tahunakademik_id) == $item->id ? 'selected' : '' }}>{{ $item->tahun_akademik }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="row">
                                <div class="col-md-6 mb-3">
                                    ...
                                    <select name="ketua_id" id="ketua_id" class="form-control select2" data-placeholder="-- Pilih Ketua --" required>
                                        ...
                                        @foreach ($users as $user)
                                            <option value="{{ $user->id }}" {{ old('ketua_id', $semesterantara->ketua_id) == $user->id ? 'selected' : '' }}>
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    <select name="sekretaris_id" id="sekretaris_id" class="form-control select2" data-placeholder="-- Pilih Sekretaris --" required>
                                        ...
                                        @foreach ($users as $user)
                                            <option value="{{ $user->id }}" {{ old('sekretaris_id', $semesterantara->sekretaris_id) == $user->id ? 'selected' : '' }}>
                                                {{ $user->name }} {{ $user->homebase ? '(' . $user->homebase . ')' : '' }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="mb-3">
                                ...
                                <textarea name="dokumen" id="dokumen" rows="3" class="form-control" placeholder="Masukkan link Google Drive dokumen LPJ Semester Antara (contoh: https://drive.google.com/file/d/...)" required>{{ old('dokumen', $semesterantara->dokumen) }}</textarea>
                                @error('dokumen')
                                    <div class="text-danger text-xs mt-1">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="d-flex align-items-center gap-2 pt-2">
                                ...
                                <a href="{{ route('semesterantara.index') }}" class="btn btn-danger text-white text-uppercase px-4 mb-0">
