                    ...
                    <div class="card-body p-3 p-md-4">
                        <form action="{{ route('skpembimbingakademik.update', $skpembimbingakademik) }}" method="POST">
                            ...
                            <div class="mb-3">
                                ...
                                <select name="user_id" id="user_id" class="form-control select2" data-placeholder="-- Pilih Dosen --" required>
                                    ...
                                    @foreach ($users as $user)
                                        <option value="{{ $user->id }}" data-homebase="{{ $user->homebase ?? '' }}" {{ old('user_id', $skpembimbingakademik->user_id) == $user->id ? 'selected' : '' }}>
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" name="prodi" id="prodi" value="{{ old('prodi', $skpembimbingakademik->prodi) }}" class="form-control" placeholder="Masukkan nama prodi">
                                @error('prodi')
                                    <div class="text-danger text-xs mt-1">{{ $message }}</div>
                                @enderror
                            </div>
