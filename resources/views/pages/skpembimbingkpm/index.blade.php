...
@section('title', 'SK Pembimbing KPM')
...
@section('content')
                ...
                <div class="card mb-4 shadow-sm border-radius-lg">
                    <div class="card-header p-3 py-3 bg-transparent border-bottom d-flex align-items-center gap-2">
                        @if (auth()->check() && auth()->user()->roles !== 'dosen')
                            <a href="{{ route('skpembimbingkpm.create') }}" class="btn btn-primary text-white text-uppercase mb-0">
                                <i class="fas fa-plus me-1"></i> Tambah SK KPM
                            </a>
    ...
    <!-- Modal Import SK Pembimbing KPM -->
            ...
            <div class="modal-content border-0 shadow-lg" style="border-radius: 16px; overflow: hidden;">
                <div class="modal-header text-white" style="background: linear-gradient(310deg, #046B26 0%, #0db846 100%); border-bottom: none; padding: 1rem 1.25rem;">
                    <h6 class="modal-title text-white font-weight-bold d-flex align-items-center mb-0" id="importModalLabel" style="font-size: 0.95rem;">
                        ...
                        Import SK Pembimbing KPM Excel
                    </h6>
                    <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <form action="{{ route('skpembimbingkpm.import') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="modal-body p-3 p-md-4">
                        ...
                        <div class="p-3 mb-3" style="background-color: #e1f5fe; border: 1px solid #b3e5fc; border-radius: 10px;">
                            <p class="mb-2 text-xs" style="line-height: 1.5; color: #01579b;">
                                Format header: <strong>tahun_akademik, nama_dosen, nomor_sk, fakultas, prodi, dokumen</strong>.
                            </p>
                            <a href="{{ route('skpembimbingkpm.template') }}" class="d-inline-flex align-items-center text-xs font-weight-bold text-decoration-none" style="color: #0288d1;">
