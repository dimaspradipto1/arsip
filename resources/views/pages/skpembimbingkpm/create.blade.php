...
@section('title', 'Tambah SK Pembimbing KPM')
            ...
            <div class="col-12">
                <div class="card mt-2 mt-md-3 shadow-sm border-radius-lg">
                    <div class="card-header pb-0 p-3 bg-transparent border-bottom">
                        <h6 class="mb-0 font-weight-bolder text-dark">Form Tambah SK Pembimbing KPM</h6>
                    </div>
                    <div class="card-body p-3 p-md-4">
                        <form action="{{ route('skpembimbingkpm.store') }}" method="POST">
                            @csrf
                            ...
                            <div class="mb-3">
                                ...
                                <select name="user_ids[]" id="user_ids" class="form-control select2" multiple="multiple" data-placeholder="-- Pilih Dosen Pembimbing --" required>
                                    @foreach ($users as $user)
                                        <option value="{{ $user->id }}" data-homebase="{{ $user->homebase ?? '' }}" {{ (is_array(old('user_ids')) && in_array($user->id, old('user_ids'))) ? 'selected' : '' }}>
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" name="prodi" id="prodi" value="{{ old('prodi') }}" class="form-control" placeholder="Masukkan nama prodi (opsional)">
                                @error('prodi')
                                    <div class="text-danger text-xs mt-1">{{ $message }}</div>
                                @enderror
                            </div>
