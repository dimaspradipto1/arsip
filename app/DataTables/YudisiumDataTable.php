...
class YudisiumDataTable extends DataTable
{
    /**
     * Build the DataTable class.
     *
     * @param QueryBuilder<Yudisium> $query Results from query() method.
     */
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->addColumn('action', function ($item) {
                ...
                $editUrl = route('yudisium.edit', $item->id);
                $deleteUrl = route('yudisium.destroy', $item->id);
                $csrf = csrf_field();
                ...
                return '
                    <div class="d-flex align-items-center justify-content-center gap-2">
                        ...
                        <form action="' . $deleteUrl . '" method="POST" class="d-inline" onsubmit="return confirm(\'Apakah Anda yakin ingin menghapus data Yudisium ini?\')">
                            ' . $csrf . '
                            ' . $method . '
     ...
     *
     * @return QueryBuilder<Yudisium>
     */
    public function query(Yudisium $model): QueryBuilder
    {
        $query = $model->newQuery()
            ->select('yudisia.*')
            ->with(['tahunakademik', 'ketua', 'sekretaris'])
            ->latest('yudisia.created_at');

        if (Auth::check()) {
            $user = Auth::user();
            if ($user->roles === 'dosen') {
                $query->where(function ($q) use ($user) {
                    $q->where('yudisia.ketua_id', $user->id)
                      ->orWhere('yudisia.sekretaris_id', $user->id);
                });
