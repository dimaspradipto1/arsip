...
class SkPengajaranDataTable extends DataTable
{
    /**
     * Build the DataTable class.
     *
     * @param QueryBuilder<SkPengajaran> $query Results from query() method.
     */
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->addColumn('action', function ($item) {
                ...
                $editUrl = route('skpengajaran.edit', $item->id);
                $deleteUrl = route('skpengajaran.destroy', $item->id);
                $csrf = csrf_field();
                ...
                return '
                    <div class="d-flex align-items-center justify-content-center gap-2">
                        ...
                        <form action="' . $deleteUrl . '" method="POST" class="d-inline" onsubmit="return confirm(\'Apakah Anda yakin ingin menghapus SK Pengajaran ini?\')">
                            ' . $csrf . '
                            ' . $method . '
     ...
     *
     * @return QueryBuilder<SkPengajaran>
     */
    public function query(SkPengajaran $model): QueryBuilder
    {
        $query = $model->newQuery()
            ->select('sk_pengajarans.*')
            ->with(['tahunakademik', 'user'])
            ->latest('sk_pengajarans.created_at');

        if (Auth::check()) {
            $user = Auth::user();
            if ($user->roles === 'dosen') {
                $query->where('sk_pengajarans.user_id', $user->id);
    ...
    {
        return 'SkPengajaran_' . date('YmdHis');
    }
}
