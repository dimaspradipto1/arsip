...
class UjianTengahSemesterDataTable extends DataTable
{
    /**
     * Build the DataTable class.
     *
     * @param QueryBuilder<UjianTengahSemester> $query Results from query() method.
     */
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->addColumn('action', function ($item) {
                ...
                $editUrl = route('ujiantengahsemester.edit', $item->id);
                $deleteUrl = route('ujiantengahsemester.destroy', $item->id);
                $csrf = csrf_field();
                ...
                return '
                    <div class="d-flex align-items-center justify-content-center gap-2">
                        ...
                        <form action="' . $deleteUrl . '" method="POST" class="d-inline" onsubmit="return confirm(\'Apakah Anda yakin ingin menghapus data Ujian Tengah Semester ini?\')">
                            ' . $csrf . '
                            ' . $method . '
     ...
     *
     * @return QueryBuilder<UjianTengahSemester>
     */
    public function query(UjianTengahSemester $model): QueryBuilder
    {
        $query = $model->newQuery()
            ->select('ujian_tengah_semesters.*')
            ->with(['tahunakademik', 'ketua', 'sekretaris'])
            ->latest('ujian_tengah_semesters.created_at');

        if (Auth::check()) {
            $user = Auth::user();
            if ($user->roles === 'dosen') {
                $query->where(function ($q) use ($user) {
                    $q->where('ujian_tengah_semesters.ketua_id', $user->id)
                      ->orWhere('ujian_tengah_semesters.sekretaris_id', $user->id);
                });
            } elseif ($user->roles !== 'admin') {
                $query->where(function ($q) use ($user) {
                    $q->whereHas('ketua', function ($sub) use ($user) {
    ...
    {
        return $this->builder()
            ->setTableId('ujiantengahsemester-table')
    ...
    {
        return 'UjianTengahSemester_' . date('YmdHis');
    }
}
