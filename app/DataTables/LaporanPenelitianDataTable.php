...
class LaporanPenelitianDataTable extends DataTable
{
    /**
     * Build the DataTable class.
     *
     * @param QueryBuilder<LaporanPenelitian> $query Results from query() method.
     */
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->addColumn('action', function ($item) {
                ...
                $editUrl = route('laporanpenelitian.edit', $item->id);
                $deleteUrl = route('laporanpenelitian.destroy', $item->id);
                $csrf = csrf_field();
                ...
                return '
                    <div class="d-flex align-items-center justify-content-center gap-2">
                        ...
                        <form action="' . $deleteUrl . '" method="POST" class="d-inline" onsubmit="return confirm(\'Apakah Anda yakin ingin menghapus data laporan penelitian ini?\')">
                            ' . $csrf . '
                            ' . $method . '
     ...
     *
     * @return QueryBuilder<LaporanPenelitian>
     */
    public function query(LaporanPenelitian $model): QueryBuilder
    {
        $query = $model->newQuery()
            ->select('laporan_penelitians.*')
            ->with(['user'])
            ->latest('laporan_penelitians.created_at');

        if (Auth::check()) {
            $user = Auth::user();
            if ($user->roles === 'dosen') {
                $query->where('laporan_penelitians.user_id', $user->id);
    ...
    {
        return $this->builder()
            ->setTableId('laporanpenelitian-table')
    ...
    public function getColumns(): array
    {
        ...
        $columns[] = Column::make('judul_penelitian')
    ...
    {
        return 'LaporanPenelitian_' . date('YmdHis');
    }
}
