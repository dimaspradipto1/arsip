...
class KartuRencanaStudiDataTable extends DataTable
{
    /**
     * Build the DataTable class.
     *
     * @param QueryBuilder<KartuRencanaStudi> $query Results from query() method.
     */
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->addColumn('action', function ($item) {
                ...
                $editUrl = route('karturencanaStudi.edit', $item->id);
                $deleteUrl = route('karturencanaStudi.destroy', $item->id);
                $csrf = csrf_field();
                ...
                return '
                    <div class="d-flex align-items-center justify-content-center gap-2">
                        ...
                        <form action="' . $deleteUrl . '" method="POST" class="d-inline" onsubmit="return confirm(\'Apakah Anda yakin ingin menghapus data Kartu Rencana Studi ini?\')">
                            ' . $csrf . '
                            ' . $method . '
     ...
     *
     * @return QueryBuilder<KartuRencanaStudi>
     */
    public function query(KartuRencanaStudi $model): QueryBuilder
    {
        $query = $model->newQuery()
            ->select('kartu_rencana_studis.*')
            ->with(['tahunakademik', 'ketuaPanitia', 'sekretaris'])
            ->latest('kartu_rencana_studis.created_at');

        if (Auth::check()) {
            $user = Auth::user();
            if ($user->roles === 'dosen') {
                $query->where(function ($q) use ($user) {
                    $q->where('kartu_rencana_studis.ketua_panitia_id', $user->id)
                      ->orWhere('kartu_rencana_studis.sekretaris_id', $user->id);
                });
    ...
    {
        return $this->builder()
            ->setTableId('karturencanaStudi-table')
    ...
    {
        return 'KartuRencanaStudi_' . date('YmdHis');
    }
}
