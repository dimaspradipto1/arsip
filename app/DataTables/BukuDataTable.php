...
class BukuDataTable extends DataTable
{
     ...
     *
     * @param QueryBuilder<Buku> $query Results from query() method.
     */
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->addColumn('action', function ($item) {
                ...
                $editUrl = route('buku.edit', $item->id);
                $deleteUrl = route('buku.destroy', $item->id);
                $csrf = csrf_field();
                ...
                return '
                    <div class="d-flex align-items-center justify-content-center gap-2">
                        ...
                        <form action="' . $deleteUrl . '" method="POST" class="d-inline" onsubmit="return confirm(\'Apakah Anda yakin ingin menghapus data buku ini?\')">
                            ' . $csrf . '
                            ' . $method . '
    ...
    public function query(Buku $model): QueryBuilder
    {
        $query = $model->newQuery()
            ->select('bukus.*')
            ->with(['user'])
            ->latest('bukus.created_at');

        if (Auth::check()) {
            $user = Auth::user();
            if ($user->roles === 'dosen') {
                $query->where('bukus.user_id', $user->id);
    ...
    public function getColumns(): array
    {
        ...
        $columns[] = Column::make('judul_buku')
            ->title('Judul Buku')
            ->addClass('align-middle text-xs');
        $columns[] = Column::make('isbn')
            ->title('ISBN')
            ->addClass('align-middle text-xs');
        $columns[] = Column::make('penerbit')
            ->title('Penerbit')
            ->addClass('align-middle text-xs');
