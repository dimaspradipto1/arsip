...
class SemesterAntaraDataTable extends DataTable
{
    /**
     * Build the DataTable class.
     *
     * @param QueryBuilder<SemesterAntara> $query Results from query() method.
     */
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->addColumn('action', function ($item) {
                ...
                $editUrl = route('semesterantara.edit', $item->id);
                $deleteUrl = route('semesterantara.destroy', $item->id);
                $csrf = csrf_field();
                ...
                return '
                    <div class="d-flex align-items-center justify-content-center gap-2">
                        ...
                        <form action="' . $deleteUrl . '" method="POST" class="d-inline" onsubmit="return confirm(\'Apakah Anda yakin ingin menghapus data Semester Antara ini?\')">
                            ' . $csrf . '
                            ' . $method . '
     ...
     *
     * @return QueryBuilder<SemesterAntara>
     */
    public function query(SemesterAntara $model): QueryBuilder
    {
        $query = $model->newQuery()
            ->select('semester_antaras.*')
            ->with(['tahunakademik', 'ketua', 'sekretaris'])
            ->latest('semester_antaras.created_at');

        if (Auth::check()) {
            $user = Auth::user();
            if ($user->roles === 'dosen') {
                $query->where(function ($q) use ($user) {
                    $q->where('semester_antaras.ketua_id', $user->id)
                      ->orWhere('semester_antaras.sekretaris_id', $user->id);
                });
    ...
    {
        return $this->builder()
            ->setTableId('semesterantara-table')
    ...
    {
        return 'SemesterAntara_' . date('YmdHis');
    }
}
