...
class KuliahPengabdianMasyarakatDataTable extends DataTable
{
    /**
     * Build the DataTable class.
     *
     * @param QueryBuilder<KuliahPengabdianMasyarakat> $query Results from query() method.
     */
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->addColumn('action', function ($item) {
                ...
                $editUrl = route('kuliahpengabdianmasyarakat.edit', $item->id);
                $deleteUrl = route('kuliahpengabdianmasyarakat.destroy', $item->id);
                $csrf = csrf_field();
                ...
                return '
                    <div class="d-flex align-items-center justify-content-center gap-2">
                        ...
                        <form action="' . $deleteUrl . '" method="POST" class="d-inline" onsubmit="return confirm(\'Apakah Anda yakin ingin menghapus data Kuliah Pengabdian Masyarakat ini?\')">
                            ' . $csrf . '
                            ' . $method . '
     ...
     *
     * @return QueryBuilder<KuliahPengabdianMasyarakat>
     */
    public function query(KuliahPengabdianMasyarakat $model): QueryBuilder
    {
        $query = $model->newQuery()
            ->select('kuliah_pengabdian_masyarakats.*')
            ->with(['tahunakademik', 'ketuaPanitia', 'sekretaris'])
            ->latest('kuliah_pengabdian_masyarakats.created_at');

        if (Auth::check()) {
            $user = Auth::user();
            if ($user->roles === 'dosen') {
                $query->where(function ($q) use ($user) {
                    $q->where('kuliah_pengabdian_masyarakats.ketua_panitia_id', $user->id)
                      ->orWhere('kuliah_pengabdian_masyarakats.sekretaris_id', $user->id);
                });
    ...
    {
        return $this->builder()
            ->setTableId('kuliahpengabdianmasyarakat-table')
    ...
    {
        return 'KuliahPengabdianMasyarakat_' . date('YmdHis');
    }
}
